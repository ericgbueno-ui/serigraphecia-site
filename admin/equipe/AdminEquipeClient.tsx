...
// Nota: campos internos (carModel, carColor, carYear, carPlate, cadastur) preservados do schema original.
// Na UI são exibidos como "Recurso/Especialidade", "Detalhe", "Registro Profissional", etc.

export default function AdminEquipeClient() {
  ...
  const handleCopyLink = () => {
    navigator.clipboard.writeText(window.location.origin + "/profissional");
    alert("Link de cadastro copiado para a área de atendimentoência!");
  };
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-wrap gap-4 items-start justify-between mb-8">
          <div>
            ...
            <h1 className="text-2xl font-bold text-white mb-1">Gestão de Profissionais</h1>
            <p className="text-sm text-white/50 mb-4">Gerencie sua equipe, avalie cadastros pendentes e visualize chaves PIX para repasses.</p>
        ...
        <div className="bg-[#111] border border-white/10 rounded-2xl overflow-hidden">
          {loading ? (
            <div className="p-12 text-center text-white/40 text-sm">Carregando profissionais...</div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-white/[0.02] border-b border-white/5">
                  ...
                  <th className="px-6 py-4 text-[10px] font-bold text-white/40 uppercase tracking-wider">Profissional</th>
                  <th className="px-6 py-4 text-[10px] font-bold text-white/40 uppercase tracking-wider hidden md:table-cell">Recurso/Especialidade</th>
                ...
                {filteredDrivers.map((driver) => (
                  <tr key={driver.id} className="hover:bg-white/[0.01] transition-colors">
                    ...
                    <td className="px-6 py-5 align-top hidden md:table-cell">
                      ...
                      <div className="text-xs text-white/50 mb-1">Detalhe: {driver.carColor}</div>
                    ...
                    <td className="px-6 py-5 align-top hidden lg:table-cell">
                      ...
                      {driver.cadastur && (
                        <div className="mt-2 text-[10px] text-white/40 uppercase tracking-widest border border-white/10 rounded px-1.5 py-0.5 inline-block">
                          Registro Profissional: {driver.cadastur}
                        </div>
                      )}
                    </td>
