...
export function NovoAgendamentoForm({
  ...
  const [cityTourDest, setCityTourDest] = useState("sua região");
  ...
  const calc = useMemo(() => {
    ...
    if (tripType === "ida_volta") {
      ...
      const volta = calcularAtendimento({
        pax,
        tripType: "volta",
        payMethod: basePayMethod,
  ...
  return (
    ...
    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2">
            <label className="text-xs text-amber-500/80 uppercase tracking-wider font-semibold">
              Parceiro Representante (Opcional)
            </label>
            <select
              name="affiliateCode"
      ...
      {/* Detalhes da Atendimento */}
      <div className="space-y-4">
        <h2 className="text-lg font-semibold border-b border-white/10 pb-2">Detalhes da Atendimento</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            >
              ...
              <option value="so_citytour">Somente Serviço Extra</option>
            </select>
          </div>
          <div>
            <label className="text-xs text-white/50 uppercase tracking-wider">Clientes</label>
            <input
              name="passengerCount"
              type="number"
              min="1"
          ...
          <div className="md:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-4">
            {tripType !== "so_citytour" ? (
              <>
                <div>
                  <label className="text-xs text-white/50 uppercase tracking-wider">
                    {tripType === "ida_volta" ? "Origem do Atendimento" : "Origem / Rota"}
                  </label>
                  <select
                    name="arrivalRouteId"
                    ...
                    className="w-full mt-1 bg-black/40 border border-white/10 rounded-lg px-4 py-2 text-sm focus:ring-1 focus:ring-emerald-500 outline-none appearance-none [color-scheme:dark]"
                  >
                    <option value="poa_gramado">📍 Unidade Principal ↔ sua região</option>
                    <option value="caxias_gramado">📍 Unidade Secundária ↔ sua região</option>
                  </select>
                </div>
            ...
            <div className="md:col-span-2">
              <label className="text-xs text-amber-400/80 uppercase tracking-wider font-semibold">
                ...
                <span className="text-white/30 normal-case font-normal">
                  (quando o atendimento não é no local padrão)
                </span>
              </label>
              <input
                name="pickupAddress"
                ...
                placeholder="Ex: Filial Centro — Av. Principal, 45"
              />
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                >
                  <option value="">—</option>
                  <option value="sedan">Equipe Padrão</option>
                  <option value="van">Equipe Ampliada</option>
                  <option value="suv">Equipe Especializada</option>
                  <option value="executivo">Equipe Premium</option>
                </select>
              </div>
            </div>
          </div>
      ...
      {tripType !== "so_citytour" && (
        <div className="space-y-4">
          <h2 className="text-lg font-semibold border-b border-white/10 pb-2">Informações do Agendamento</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-3">
              <h3 className="text-sm font-medium text-blue-400">Início do Atendimento</h3>
              <div>
                <label className="text-xs text-white/50 uppercase tracking-wider">
                  Data de Início
                </label>
              ...
              <div className="flex gap-2">
                ...
                <div className="flex-1">
                  <label className="text-xs text-white/50 uppercase tracking-wider">Referência (opcional)</label>
                  <input
                    name="idaFlight"
                    className="w-full mt-1 bg-black/40 border border-white/10 rounded-lg px-4 py-2 text-sm focus:ring-1 focus:ring-emerald-500 outline-none"
                    placeholder="Ex: cód. interno"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <h3 className="text-sm font-medium text-purple-400">Encerramento do Atendimento</h3>
              <div>
                <label className="text-xs text-white/50 uppercase tracking-wider">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {/* Serviço Premium — só para unidade principal */}
          ...
          {/* Itens/equipamentos adicionais por tipo */}
          <div className="md:col-span-2 bg-black/20 border border-white/5 rounded-xl p-4 space-y-3">
            <div>
              <p className="text-sm text-white/90 font-medium">
                🧰 Itens e Equipamentos Adicionais
              </p>
              <p className="text-xs text-white/40 mt-0.5">
                Quantidade de itens/equipamentos extras necessários para este atendimento.
              </p>
            </div>
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          placeholder="Notas visíveis apenas para a equipe: rota especial, observações da agendamento, acordos específicos..."
        />
      ...
      <div className="pt-4">
        ...
        >
          {submitting ? "Registrando..." : "Registrar Agendamento Manual"}
        </button>
      </div>
    </form>
  );
