  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (isIda) {
      ...
      if (driverInName) {
        ...
        if (!driverInWhatsapp) missing.push("WhatsApp do Profissional");
        
        const payVal = parseFloat(driverInPayment.replace(",", "."));
        if (!driverInPayment || isNaN(payVal) || payVal <= 0) {
          missing.push("Valor pago ao profissional (deve ser maior que R$ 0,00)");
        }

        if (missing.length > 0) {
          alert(`⚠️ Trecho de CHEGADA (IN):\nPara designar o profissional "${driverInName}", você também precisa preencher:\n\n- ${missing.join("\n- ")}`);
          return;
        }
      }
    }

    // ── Encerramento Validação ───────────────────────────────────────────────
    if (isVolta) {
      ...
      if (driverOutName) {
        ...
        if (!driverOutWhatsapp) missing.push("WhatsApp do Profissional");
        ...
        if (missing.length > 0) {
          alert(`⚠️ Trecho de RETORNO (OUT):\nPara designar o profissional "${driverOutName}", você também precisa preencher:\n\n- ${missing.join("\n- ")}`);
          return;
        }
      }
    }

    // ── Serviço Adicional Validação ──────────────────────────────────────────────────
    if (cityTour?.enabled) {
      ...
      if (driverCityTourName) {
        ...
        if (!driverCityTourWhatsapp) missing.push("WhatsApp do Profissional");
        ...
        if (missing.length > 0) {
          alert(`⚠️ Trecho de SERVIÇO ADICIONAL:\nPara designar o profissional "${driverCityTourName}", você também precisa preencher:\n\n- ${missing.join("\n- ")}`);
          return;
        }
      }
    }
    ...
    startTransition(async () => {
      try {
        ...
        console.error("[ProfissionalForm] Erro ao salvar profissionais:", err);
      }
    });
  }
            ...
            >
              ...
              >
                <p style={{ fontSize: "11px", color: "var(--muted)", marginBottom: "8px", fontWeight: 600 }}>
                  Comprovante para o profissional (Copie e cole):
                </p>
                <textarea
                  readOnly
                  style={{
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px" }}>
                <div style={{ gridColumn: "1 / -1", marginBottom: "4px" }}>
                  <label style={label}>Selecione um Profissional da Frota (Opcional)</label>
                  <select
                  ...
                  >
                    <option value="">-- Escolher profissional aprovado --</option>
                    {drivers?.map(d => (
                ...
                <div>
                  <label style={label}>Nome do Profissional</label>
                  <input
                    name="driverInName"
                    value={inName}
                ...
                <div>
                  <label style={label}>WhatsApp do Profissional</label>
                  <input
              ...
              <div style={{ marginTop: "12px" }}>
                <label style={{ ...label, color: "#f87171" }}>💸 Valor pago ao profissional</label>
        ...
        >
          <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
            ...
            >
              ...
              />
              Enviar WhatsApp para o cliente com os dados do profissional
            </label>
          ...
          >
            {isPending ? "⏳ Salvando..." : "💾 Salvar profissionais"}
          </button>
        </div>
      </form>
    </div>
