...
export default async function AdminReservasPage({
  ...
  const monthWhere = {
    OR: [
      ...
      // Sistema novo: contrato feito neste mês, com atendimento em mai/2026+
      {
  ...
  const [allMonthBookings, aReceberBookings, bookings] = await Promise.all([
    ...
    prisma.booking.findMany({
      where: monthWhere,
      select: { status: true, totalCents: true, remainderCents: true, payMethod: true, noShowFlightType: true },
    }),
  ...
  const noShowCountMes = allMonthBookings.filter((b) => b.noShowFlightType).length;
  ...
  // Obter a data mais antiga pelo idaDate (data de atendimento)
      ...
      <div style={{ maxWidth: "1200px" }}>
        ...
        >
          {[
            ...
            {
              label: "NoShows (Mês)",
              value: `${noShowCountMes}`,
              sub: confirmedCount > 0 ? `${Math.round((noShowCountMes / confirmedCount) * 100)}% dos contratos confirmados` : "—",
              color: "#f87171",
        ...
        {bookings.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            {bookings.map((b) => {
              ...
              const hasNoShow = !!b.noShowFlightType;
              return (
                <Link
                  key={b.id}
                  ...
                  style={{
                    background: hasNoShow ? "rgba(248,113,113,0.06)" : "var(--bg-card)",
                    border: hasNoShow ? "1px solid rgba(248,113,113,0.35)" : "1px solid var(--border)",
                    borderRadius: "14px",
                    padding: "18px 24px",
                    display: "grid",
                ...
                >
                  ...
                  <div>
                    ...
                    {b.idaDate && (
                      <p style={{ fontSize: "11px", color: "var(--text)", fontWeight: 500, marginTop: "4px" }}>
                        🗓️ Início: {formatDate(b.idaDate)}
                        {b.idaFlightTime ? ` às ${b.idaFlightTime}` : ""}
                        {b.idaFlightNumber ? ` (Ref. ${b.idaFlightNumber})` : ""}
                      </p>
                    )}
                    {b.voltaDate && (
                      <p style={{ fontSize: "11px", color: "var(--text)", fontWeight: 500, marginTop: "2px" }}>
                        ↩️ Encerramento: {formatDate(b.voltaDate)}
                        {b.voltaFlightTime ? ` às ${b.voltaFlightTime}` : ""}
                        {b.voltaFlightNumber ? ` (Ref. ${b.voltaFlightNumber})` : ""}
                      </p>
                    )}
                  </div>

                  {/* Col 3: Valor + representante */}
                  <div>
                    ...
                    >
                      {b.payMethod === "pix" ? "PIX" : b.payMethod === "pix_50" ? "PIX 50%" : b.payMethod === "cartao" ? "Cartão" : "Manual"}
                    </p>
                    {b.affiliateCode && (
                  ...
                  >
                    ...
                    {hasNoShow && (
                      ...
                      >
                        🚨 NoShow {b.noShowFlightType === "ida" ? "IN" : "OUT"}
                        {b.rescheduledDate ? " · remarcado" : ""}
                      </span>
                    )}
                    {b.noShowOutCancelled && (
                      <span
                        style={{
                          display: "inline-flex",
                          alignItems: "center",
                      ...
                      >
                        ❌ Encerramento cancelado{b.outCancelledCents ? ` (-${brl(b.outCancelledCents)})` : ""}
                      </span>
                    )}
          ...
          >
            {Array.from({ length: totalPages }).map((_, i) => {
              ...
              return (
                <Link
                  key={p}
                  href={`/admin/agendamentos?month=${selectedMonth}&page=${p}`}
                  style={{
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
