...
export const metadata = {
  title: "Relatório de Motoristas | Admin Multi Trip",
...
export default async function AdminCaixaMotoristasPage() {
  await requireAdmin();
  ...
  // 1. Corridas Pagas (com gastos de motorista maiores que zero)
  const paidRides = allDriversRides
    .filter((r) => r.total > 0)
    .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());

  // 2. Corridas Pendentes (motorista designado mas custo zerado)
  const pendingRides = allDriversRides
    .filter((r) => {
      const hasDriver = !!r.driverInName || !!r.driverOutName || !!r.driverCityTourName;
      // Se tem motorista escalado em algum trecho, mas o pagamento daquele trecho específico está zerado
      ...
      <div>
        ...
        <div className="caixa-header">
          <div className="caixa-title-block">
            <h1>Relatório Detalhado de Motoristas</h1>
            <p>Controle de despesas, repasses e trechos efetuados por reserva confirmada.</p>
          </div>
        ...
        <div className="driver-kpi-grid">
          <div className="driver-kpi-card" style={{ borderTop: "3px solid rgba(248,113,113,0.3)" }}>
            ...
            <p className="driver-kpi-sub">Total repassado em todas as reservas confirmadas</p>
          </div>
        ...
        <div className="notice-box">
          💡 <strong>Como funciona o controle de despesas de motoristas?</strong> Para que o sistema compute uma despesa com motorista no fluxo de caixa, é necessário preencher o valor do pagamento (em R$) no trecho correspondente dentro da reserva. Atribuir apenas o nome do motorista sem definir o valor pago a ele resulta em R$ 0,00 e o sistema não registrará despesa. Use a tabela de pendências abaixo para encontrar corridas que necessitam de definição de custo.
        </div>

        {/* ── Tabela 1: Planilha Detalhada de Pagamentos (Gastos Efetuados) ── */}
        <h2 className="section-title">
          <span>📊</span> Planilha de Repasses de Motoristas ({paidRides.length})
        </h2>
        <div className="report-table-wrapper">
          {paidRides.length === 0 ? (
            <div className="empty-state">
              Nenhuma despesa com motorista computada ainda.
              <br />
              <span style={{ fontSize: "11px", display: "block", marginTop: "4px" }}>
                Preencha os valores nas reservas dos clientes para gerar despesas.
              </span>
            </div>
          ) : (
            <table className="report-table">
              <thead>
                <tr>
                  ...
                  <th>Chegada (IN)</th>
                  <th className="col-hide">Retorno (OUT)</th>
                  <th className="col-hide">City Tour</th>
                  <th>Total Pago</th>
                  <th>Ações</th>
                </tr>
              </thead>
              <tbody>
                {paidRides.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td>
                      <Link href={`/admin/reservas/${r.id}`} className="btn-edit">
                        Editar
                      </Link>
                    </td>
                  </tr>
        ...
        {/* ── Tabela 2: Corridas com Motoristas Escalados sem Custo Definido ── */}
        <h2 className="section-title" style={{ marginTop: "44px" }}>
          <span style={{ color: "#fb923c" }}>⚠️</span> Corridas com Motoristas Escalados sem Custo Definido ({pendingRides.length})
        </h2>
        <div className="report-table-wrapper" style={{ border: "1px solid rgba(251, 146, 60, 0.2)" }}>
          {pendingRides.length === 0 ? (
            <div className="empty-state">
              🎉 Todas as corridas com motoristas escalados possuem custos definidos!
            </div>
          ) : (
            <table className="report-table">
              <thead>
                <tr>
                  <th>Cliente / Reserva</th>
                  <th>Segmentos & Motoristas Escalados</th>
              ...
              <tbody>
                {pendingRides.map((r) => {
                  ...
                  if (r.driverInName && r.inPayment === 0) {
                    pendingSegments.push(`Chegada (IN): ${r.driverInName}`);
                  }
                  if (r.driverOutName && r.outPayment === 0) {
                    pendingSegments.push(`Retorno (OUT): ${r.driverOutName}`);
                  }
                  if (r.driverCityTourName && r.cityTourPayment === 0) {
                    pendingSegments.push(`City Tour: ${r.driverCityTourName}`);
                  }
                  ...
                  return (
                    <tr key={r.id}>
                      ...
                      <td>
                        <Link href={`/admin/reservas/${r.id}`} className="btn-edit" style={{ color: "#fb923c", background: "rgba(251, 146, 60, 0.06)", borderColor: "rgba(251, 146, 60, 0.2)" }}>
                          Definir Valor →
                        </Link>
                      </td>
                    </tr>
