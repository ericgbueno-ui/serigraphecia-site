...
export async function createManualBooking(formData: FormData) {
  ...
  const email =
    formData.get("email")?.toString().trim() || `cliente.${Date.now()}@multitrip.com.br`;
  ...
  if (tripType === "so_citytour") {
    ...
    const routeContext =
      tripType === "volta"
        ? canonicalTransferRoute("volta", routeId)
        : tripType === "ida_volta"
          ? canonicalTransferRoute("ida_volta", routeId)
          : canonicalTransferRoute("ida", routeId);
