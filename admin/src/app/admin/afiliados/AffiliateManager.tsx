...
export function AffiliateManager({ data }: { data: ParsedAffiliate[] }) {
  ...
  function openPay(aff: ParsedAffiliate) {
    if (aff.pendingComm <= 0) {
      alert("Este afiliado não tem saldo pendente para receber.");
      return;
    }
    setSelectedAff(aff);
  ...
  function affiliateUrl(code: string) {
    return `multitrip.com.br/${code.toLowerCase()}`;
  }
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead>
              <tr className="border-b border-white/10 text-white/40 bg-black/40">
                <th className="px-6 py-4 font-semibold tracking-wide">Afiliado</th>
                <th className="px-6 py-4 font-semibold tracking-wide">Link do afiliado</th>
              ...
              {filteredAffiliates.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-6 py-16 text-center text-white/40">
                    <div className="text-lg">Nenhum afiliado encontrado nesta categoria.</div>
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <div className="bg-[#111] border border-white/10 p-6 rounded-2xl w-full max-w-sm shadow-2xl">
            ...
            <form onSubmit={handleSavePayment} className="space-y-4">
              <div className="bg-yellow-500/10 border border-yellow-500/20 p-3 rounded-lg text-sm text-yellow-300">
                O afiliado possui <strong>{brl(selectedAff.pendingComm)}</strong> aguardando
                pagamento.
              </div>
              <div>
