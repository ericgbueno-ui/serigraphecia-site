...
function redirectWithError(message: string) {
  redirect(`/admin/afiliados/novo?error=${encodeURIComponent(message)}`);
}
...
export async function createAffiliate(formData: FormData) {
  ...
  if (existing) {
    redirectWithError("Já existe um afiliado com esse código.");
  }
  ...
  redirect(`/admin/afiliados/${affiliate.id}`);
}
