...
type TimelinePoint = { date: string; leads: number; reservas: number };
...
function TimelineChart({ data }: { data: TimelinePoint[] }) {
  ...
  const maxRes = Math.max(...data.map((d) => d.reservas), 1);
  ...
  const leadsPath = data
    ...
    .join(" ");
  const resPath = data
    .map((d, i) => `${i === 0 ? "M" : "L"}${xPos(i).toFixed(1)},${yPos(d.reservas).toFixed(1)}`)
    .join(" ");
  ...
  return (
    <div style={{ overflowX: "auto" }}>
      ...
      >
        ...
        {/* Dots reservas */}
        {data.map((d, i) => (
          <circle key={`r${i}`} cx={xPos(i)} cy={yPos(d.reservas)} r="2.5" fill="#4ade80" />
        ))}
      </svg>
...
function buildDiagnostico(data: AnalyticsData): DiagItem[] {
  ...
  // LTV igual ao AOV → cliente compra só uma vez
  if (m.aov > 0 && Math.abs(m.ltv - m.aov) < 1) {
    items.push({
      level: "atencao",
      titulo: "Clientes comprando apenas uma vez",
      descricao: "LTV igual ao ticket médio indica que não há recompra registrada. Retenção zerada.",
      acao: `Ativar follow-up D+30 oferecendo desconto para próxima viagem. Criar programa de fidelidade simples (ex: "3ª viagem com desconto").`,
    });
  }
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          ...
          >
            ...
            <KpiCard
              ...
              value={fmt(data.kpis.reservas.periodo)}
              sub={`Total confirmados: ${fmt(data.kpis.reservas.confirmadas)}`}
            />
            <KpiCard
              label={`Faturamento (${period}d)`}
              value={brl(data.kpis.reservas.faturamentoPeriodo)}
              sub={`Total: ${brl(data.kpis.reservas.faturamentoTotal)}`}
              color="#4ade80"
              highlight
            />
            <KpiCard
            ...
            <div style={card}>
              ...
              >
                {[
                  ...
                  { label: "Reservas Ads", value: fmt(data.metaAds.conversoes), onClick: () => setShowMetaBookingsModal(true) },
                  ...
                  { label: "CPA (Custo/Reserva)", value: brl(data.metaAds.cpa * 100) },
          ...
          >
            ...
            >
              <h3 style={{ fontSize: "16px", fontWeight: 700, color: "var(--text)" }}>
                🎯 Reservas Atribuídas ao Meta Ads
              </h3>
              <button
                onClick={() => setShowMetaBookingsModal(false)}
                style={{
            ...
            <div style={{ padding: "20px 24px", overflowY: "auto", flex: 1 }}>
              {data.metaAds.bookings && data.metaAds.bookings.length > 0 ? (
                <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                  ...
                  >
                    <span>Total de reservas: <strong>{data.metaAds.bookings.length}</strong></span>
                ...
                <div style={{ textAlign: "center", padding: "40px 0", color: "var(--muted)" }}>
                  Nenhuma reserva do Meta Ads registrada no período de {period} dias.
                </div>
              )}
            </div>
