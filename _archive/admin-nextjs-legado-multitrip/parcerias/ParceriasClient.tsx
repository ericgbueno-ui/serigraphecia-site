...
export default function ParceriasClient() {
  ...
  async function handleImport() {
    ...
    const data = await res.json();
    if (res.ok) { setMsg(`✅ ${data.imported} fornecedores importados.`); await load(); }
    ...
    <div style={{ padding: "32px 40px", maxWidth: "1100px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "24px", flexWrap: "wrap", gap: "12px" }}>
        <div>
          ...
          <h1 style={{ fontSize: "22px", fontWeight: 700, color: "var(--text)" }}>💼 CRM de Parcerias B2B / Fornecedores</h1>
        ...
        {contacts.length === 0 && (
          <button onClick={handleImport} disabled={importing}
            ...
            {importing ? "Importando..." : "⬇️ Importar fornecedores"}
          </button>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div style={{ ...card, textAlign: "center", padding: "60px 24px" }}>
          ...
          <p style={{ fontSize: "13px", color: "var(--muted)" }}>
            {contacts.length === 0 ? 'Clique em "Importar fornecedores" para começar.' : "Tente outro filtro."}
          </p>
        </div>
      ) : (
      ...
      {contacts.filter((c) => c.whatsapp && c.status === "a_enviar").length > 0 && (
        <div style={{ marginTop: "20px", background: "rgba(180,140,60,0.06)", border: "1px solid var(--gold-line)", borderRadius: "12px", padding: "16px 20px", fontSize: "12px", color: "var(--muted)", lineHeight: "1.7" }}>
          ...
          Copie os números abaixo e importe manualmente no WhatsApp do negócio como lista de transmissão:
