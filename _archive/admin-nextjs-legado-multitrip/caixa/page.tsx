...
export const metadata = {
  title: "Fluxo de Caixa | Admin [NOME DO NEGÓCIO]",
...
// Corte do sistema: atendimentos com data antes de maio/2026 são históricos
const SYSTEM_START = new Date("2026-05-01T00:00:00.000Z");

// Histórico (data antes de mai/2026): grupo pela data do atendimento.
// Novo (data em mai/2026+): grupo pelo mês de criação do contrato.
...
// A Receber: sempre pelo mês da data do atendimento (quando o cliente paga o restante)
...
// Nota: valores de "v" (sedan, van, executivo, suv, suv_eletrico, roteiro) são
// identificadores internos de categoria de serviço preservados do schema original.
// Os rótulos abaixo são apenas o texto exibido na tela.
function vehicleLabel(v: string) {
  const map: Record<string, string> = {
    ...
    executivo: "Executivo",
    suv: "Premium",
    suv_eletrico: "Premium Plus",
  };
  return map[v] ?? v;
}

function tripLabel(t: string) {
  if (t === "so_citytour") return "Serviço Adicional";
  if (t === "ida_volta") return "Atendimento Completo";
...
export default async function AdminCaixaPage() {
  ...
  // ── Gastos com Profissionais — lê de optionalsJson (sem migration) ──────────
  ...
  // Agendamentos confirmadas do mês atual (para CPA) — mesmo filtro híbrido
  ...
  // ── Serviços Adicionais e Cashbacks Calculations ──
  const standardRoteiros = bookings.filter(
    (b) => b.vehicleType === "roteiro" && b.totalCents === 2990
  );
  const curadoriaAssistenteBookings = bookings.filter(
  ...
  const curadoriaQty = curadoriaAssistenteBookings.length;
  const curadoriaRevenue = curadoriaAssistenteBookings.reduce((sum, b) => sum + b.totalCents, 0);

  // Total cashbacks gerados: R$ 150 para cada Curadoria Assistente
  const cashbackGeneratedCents = curadoriaQty * 15000;

  // Somar todos os cashbacks resgatados em todas as agendamentos confirmadas
  ...
  // A Receber: agendamentos com chegada neste mês (quando o cliente paga o restante)
  ...
  const pendingBookingsMes = bookingsArrivingThisMonth
    ...
    .map((b) => ({
      ...
      cityTourDate: (() => { try { const o = JSON.parse(b.optionalsJson ?? "{}"); return o?.cityTour?.date ?? null; } catch { return null; } })(),
      tripType: b.tripType,
      payMethod: b.payMethod,
    }))
    .sort((a, b) => {
      const da = (a.tripType === "volta" ? a.voltaDate : a.tripType === "so_citytour" ? (a.idaDate ?? a.cityTourDate) : a.idaDate) ?? "9999";
      const db_ = (b.tripType === "volta" ? b.voltaDate : b.tripType === "so_citytour" ? (b.idaDate ?? b.cityTourDate) : b.idaDate) ?? "9999";
    ...
    <div className="caixa-page">
      ...
      <div>
        ...
        <div className="caixa-header">
          <div>
            ...
            <p style={{ fontSize: "13px", color: "var(--muted)" }}>
              Entradas das agendamentos confirmadas — separado por período.
            </p>
          </div>
          <Link
          ...
          {[
            {
              ...
              sub: `${bookingsCreatedThisMonth.length} agendamentos confirmadas`,
              color: "var(--text)",
              accent: "var(--border)",
            },
            {
            ...
            null, // substituído por AReceberCard abaixo
            {
              ...
              sub: "por agendamento confirmada",
              color: "#a78bfa",
        ...
        <div style={{ marginBottom: "32px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "16px", marginBottom: "16px" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "4px", textAlign: "right" }}>
                ...
                <span style={{ fontSize: "12px", color: "#f87171" }}>
                  − Profissionais: <strong>{brl(driverPaymentMonth)}</strong>
                </span>
          ...
          <div>
            <Link
              href="/admin/caixa/profissionais"
              style={{
                fontSize: "13px",
                fontWeight: 600,
                color: "var(--text)",
            ...
            >
              📋 Ver Relatório Detalhado e Auditoria de Profissionais →
            </Link>
          </div>
        </div>
          ...
          >
            ...
            >
              <div>
                ...
                <p style={{ fontSize: "11px", color: "var(--muted)", marginTop: "4px" }}>
                  Gera R$ 150,00 de bônus / cashback para Atendimentos
                </p>
              </div>
              <div style={{ marginTop: "16px", borderTop: "1px solid rgba(255, 255, 255, 0.05)", paddingTop: "12px", display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                <div>
            ...
            >
              ...
              <p style={{ fontSize: "12px", color: "var(--muted)", lineHeight: 1.5 }}>
                Os créditos de cashback gerados na contratação do serviço personalizado (**Curadoria Assistente**) têm o prazo estrito de validade de **12 meses (1 ano)** a contar da data de compra do serviço. Decorrido este prazo, o saldo expira automaticamente caso não tenha sido resgatado na contratação de atendimentos ou serviços adicionais.
              </p>
            </div>
          </div>
        </div>
        ...
        >
          Entradas (Agendamentos Confirmadas)
        </h2>
        ...
        {bookings.length === 0 ? (
          ...
          >
            ...
            >
              {["Cliente", "Tipo / Categoria", "Total", "Recebido", "A Receber", ""].map((h) => (
                <span
                  key={h}
                  style={{
                    fontSize: "10px",
                ...
                >
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: "11px", color: "var(--muted)", marginTop: "2px" }}>
                      ...
                      {b.idaDate ? ` · 📅 ${formatDate(b.idaDate)}` : ""}
                    </p>
                  </div>
